...
void input_binary(char* filename){
  ...
  printf("before reading N:%ld\n",ftell(inf));
  fread(&_N,sizeof(int),1,inf);

  fseek(inf, sizeof(int), SEEK_SET);
  printf("before reading t:%ld\n",ftell(inf));
  ...
  printf("size:particle %ld, particles %ld\n",sizeof(struct particle),sizeof(particles));

  int i;
  for (i=0;i<_N;i++){
    struct particle p;

    fseek(inf, sizeof(int)+sizeof(double)+i*sizeof(struct particle), SEEK_SET);
  ...
  /* for(i=0;i<10;i++){ */
  /*   printf("read:particles.xyz:%lf %lf %lf\n",particles[i].x,particles[i].y,particles[i].z); */
  /* } */
